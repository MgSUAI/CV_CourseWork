...
def compute_descriptors():
    ...
    print(f"Computing {config.descriptor_type} descriptors...")
    ...
    for image_path in tqdm(sorted(glob(f"{image_folder}/*.bmp")), desc="    Processing"):
        if image_path.endswith(".bmp"):
            ...
            if  config.descriptor_type == 'avg_rgb':
                F = extract_descriptors.avg_rgb(image_path)
            elif config.descriptor_type == 'global_color_histogram':
                F = extract_descriptors.color_histogram(image, config.num_bins)

            elif config.descriptor_type == 'grid_color_histogram':
                F = extract_descriptors.compute_grid_color_histogram(image_path,
                                                                     config.num_bins,
                                                                     config.grid_rows, 
                                                                     config.grid_cols)
            elif config.descriptor_type == 'eoh':
                F = extract_descriptors.compute_eoh(image_path,
                                                    config.eoh_bins,
                                                    config.grid_rows, 
                                                    config.grid_cols)
            elif config.descriptor_type == 'eoh_plus_ch':
                ...
                F_gch = F_gch * config.weight_color
                F_eoh = F_eoh * config.weight_eoh
                F = np.concatenate([F_gch, F_eoh])

            else:
                raise ValueError(f"Unknown descriptor type: {config.descriptor_type}")
